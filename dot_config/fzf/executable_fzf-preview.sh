...
if [[ "$file" =~ \.kra$ ]] && command -v unzip >/dev/null; then
    tmp_preview="/tmp/fzf-preview-$USER-$$.png"
    if unzip -p "$file" mergedimage.png > "$tmp_preview" 2>/dev/null || \
       unzip -p "$file" preview.png > "$tmp_preview" 2>/dev/null; then
        if [[ -s "$tmp_preview" ]]; then
            file="$tmp_preview"
